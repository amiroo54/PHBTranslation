A tool helps you make specialized ability checks, craft certain items, or both. A tool's description includes the tool's cost and weight, as well as the following entries:
{{Tool}}ها اجازه می‌دهند {{AbilityChecks}} تخصصی بیاندازی، {{Item}}‌های خاصی را بسازی یا هر دو. توضیحات هر {{Tool}} شامل هزینه و وزن آن و همچنین موارد زیر می‌باشد:
- Ability. This entry lists the ability to use when making an ability check with the tool.
- **{{Ability}}.** این مورد {{Ability}}‌ای که از آن برای انداختن {{AbilityCheck}} با این {{Tool}} استفاده می‌کنی یا نشان می‌دهد.
- **{{Utilize}}:** This entry lists things you can do with the tool when you take the Utilize] action. You can do one of those things each time you take the action. This entry also provides the {{DC}} for the action.
- **{{Utilize}}.** این مورد کارهایی را که با استفاده از {{Action}} {{Utilize}} با این {{Tool}} می‌توانی انجام بدهی را نشان می‌دهد. می‌توانی با هر استفاده از {{Action}} یکی از آن‌ها را انجام بدهی. این مورد همچنین {{DC}} آن {{Action}} را نشان می‌دهد.
- **{{Craft}}:** This entry lists what, if anything, you can craft with the tool. For crafting rules, see "Crafting Equipment](http://127.0.0.1:5050/book.html#xphb,6,crafting%20equipment,0)" later in the chapter.
- **{{Crafting}}.** این مورد نشان می‌دهد چه چیزی را می‌توانی با این {{Tool}} بسازی. برای قوانین {{Crafting}} به بخش {{CraftingEquipment}} در همین فصل مراجعه شود. 
- Variants. This entry appears if the tool has variants, which are listed. Each requires a separate proficiency.
- **انواع.** این مورد در صورتی که {{Tool}} انواع خاصی داشته باشد ظاهر می‌شود. هر کدام از این انواع نیازمند داشتن {{Proficiency}} جدا هستند. 

### {{ToolProficiency}}
اگر با یک {{Tool}} {{Proficiency}} داشته باشی، می‌توانی {{ProficiencyBonus}}ت را به هر {{AbilityCheck}}ی که با آن {{Tool}} می‌اندازی اضافه کنی. اگر در {{Skill}}ی که در آن {{AbilityCheck}} استفاده می‌شود هم {{Proficiency}} داشته باشی، برای آن {{AbilityCheck}} {{Advantage}} هم می‌گیری. 
If you have proficiency with a tool, add your Proficiency Bonus to any ability check you make that uses the tool. If you have proficiency in a skill that's used with that check, you have Advantage](http://127.0.0.1:5050/variantrules.html#advantage_xphb) on the check too.
{{Feature}}‌هایت ممکن است در استفاده از یک {{Tool}} به تو {{Proficiency}} بدهند. هر {{Monster}}ی با هر {{Tool}} در {{StatBlock}}ش {{Proficiency}} دارد.
Your features might give you proficiency with a tool. A monster has proficiency with any tool in its stat block.

### {{ArtisansTools}}

Artisan's Tools are each focused on crafting items and pursuing a trade. Each of these tools requires a separate proficiency.
هر کدام از {{ArtisanTools}} بر {{Crafting}} {{Item}}‌ها و دنبال کردن یک شغل تمرکز دارند. هر کدام از این {{Tool}}‌ها نیازمند یک {{Proficiency}} جداست.

#### {{AlchemistsSupplies}} ({{GP:۵۰}})

- **{{Ability}}**: {{Intelligence}}
- **{{Weight}}:** 8 {{LB:8}}
- **{{Utilize}}:** شناسایی یک ماده ({{DC}} 15), یا روشن کردن یک آتش ({{DC}} 15)
- **{{Craft}}:** {{Acid}}, {{AlchemistsFire}}, {{ComponentPouch}}, {{Oil}}, {{Paper}}, {{Perfume}}

#### {{BrewersSupplies}} ({{GP:۲۰}})

- **{{Ability}}:** {{Intelligence}}
- **{{Weight}}:** 9 {{LB:9}}
- **{{Utilize}}:** شناسایی نوشیدنی مسموم شده ({{DC}} 15), یا شناسایی الکل ({{DC}} 10)
- **{{Craft}}:** {{Antitoxin}}

#### {{CalligraphersSupplies}} ({{GP:۱۰}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** 5 {{LB:5}}
- **{{Utilize}}:** نوشتن با خط نستعلیق خیره‌کننده‌ای که در مقابل جعل محافظت می‌کند ({{DC}} 15)
- **{{Craft}}:** {{Ink}}, {{SpellScroll}}

#### {{CarpentersTools}} ({{GP:۸}})

- **{{Ability}}:** {{Strength}}
- **{{Weight}}:** 6 {{LB:6}}
- **{{Utilize}}:** قفل کردن یا به زور باز کردن یک در یا محفظه ({{DC}} 20)
- **{{Craft}}:** {{Club}}, {{Greatclub}}, {{Quarterstaff}}, {{Barrel}}, {{Chest}}, {{Ladder}}, {{Pole}}, {{PortableRam}}, {{Torch}}

#### {{CartographersTools}} ({{GP:۱۵}})

- **{{Ability}}:** {{Wisdom}}
- **{{Weight}}:** 6 {{LB:6}}
- **{{Utilize}}:** کشیدن یک نقشه کلی از یک ناحیه کوچک ({{DC}} 15)
- **{{Craft}}:** {{Map}}

#### {{CobblersTools}} ({{GP:۵}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** 5 {{LB:5}}
- **{{Utilize}}:** بهبود پاپوش برای دادن {{Advantage}} به تاس {{Dexterity}} ({{Acrobatics}}) بعدی کسی که آن را پوشیده ({{DC}} 10)
- **{{Craft}}:** {{ClimbersKit}}

#### {{CooksUtensils}} ({{GP:۱}})

- **{{Ability}}:** {{Wisdom}}
- **{{Weight}}:** 8 {{LB:8}}
- **{{Utilize}}:** بهبود دادن مزه غذا ({{DC}} 10), یا شناسایی غذای مسموم شده ({{DC}} 15)
- **{{Craft}}:** {{Rations}}

#### {{GlassblowersTools}} ({{GP:۳۰}})

- **{{Ability}}:** {{Intelligence}}
- **{{Weight}}:** 5 {{LB:5}}
- **{{Utilize}}:** تشخیص دادن چیزی که یک ظرف شیشه‌ای در ۲۴ ساعت گذشته در خود داشته ({{DC}} 15)
- **{{Craft}}:** {{GlassBottle}}, {{MagnifyingGlass}}, {{Spyglass}}, {{Vial}}

#### JewelersTools ({{GP:۲۵}})

- **{{Ability}}:** {{Intelligence}}
- **{{Weight}}:** {{LB:2}}
- **{{Utilize}}:** تشخیص دادن ارزش یک سنگ قیمتی ({{DC}} 15)
- **{{Craft}}:** {{ArcaneFocus}}, {{HolySymbol}}

#### {{LeatherworkersTools}} ({{GP:۵}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** {{LB:5}}
- **{{Utilize}}:** اضافه کردن یک طرح به یک شی چرمی ({{DC}} 10)
- **{{Craft}}:** {{Sling}}, {{Whip}}, {{HideArmor}}, {{LeatherArmor}}, {{StuddedLeatherArmor}}, {{Backpack}}, {{CrossbowBoltCase}}, {{MapOrScrollCase}}, {{Parchment}}, {{Pouch}}, {{Quiver}}, {{Waterskin}}

#### {{MasonsTools}} ({{GP:۱۰}})

- **{{Ability}}:** {{Strength}}
- **{{Weight}}:** {{LB:8}}
- **{{Utilize}}:** حک کردن یک نماد یا سوراخ در سنگ ({{DC}} 10)
- **{{Craft}}:** {{BlockAndTackle}}

#### {{PaintersSupplies}} ({{GP:۱۰}})

- **{{Ability}}:** {{Wisdom}}
- **{{Weight}}:** {{LB:5}}
- **{{Utilize}}:** کشیدن یک تصویر قابل تشخیص از چیزی که دیده‌ای ({{DC}} 10)
- **{{Craft}}:** {{DruidicFocus}}, {{HolySymbol}}

#### {{PottersTools}} ({{GP:۱۰}})

- **{{Ability}}:** {{Intelligence}}
- **{{Weight}}:** {{LB:3}}
- **{{Utilize}}:** تشخیص دادن چیزی که یک شی سرامیکی در ۲۴ ساعته گذشته در خود داشته ({{DC}} 15)
- **{{Craft}}:** {{Jug}}, {{Lamp}}

#### {{SmithsTools}} ({{GP:۲۰}})

- **{{Ability}}:** {{Strength}}
- **{{Weight}}:** {{LB:8}}
- **{{Utilize}}:** به زور باز کردن یک در یا محفظه ({{DC}} 20)
- **{{Craft}}:** هر {{MeleeWeapon}}ی (به جز {{Club}}, {{Greatclub}}, {{Quarterstaff}}, و {{Whip}}), {{MediumArmor}} (به جز {{HideArmor}}), {{HeavyArmor}}, {{BallBearing}}, {{Bucket}}, {{Caltrops}}, {{Chain}}, {{Crowbar}}, {{FirearmBullets}}, {{GrapplingHook}}, {{IronPot}}, {{IronSpikes}}, {{SlingBullet}}

#### {{TinkersTools}} ({{GP:۵۰}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** {{LB:10}}
- **{{Utilize}}:** سرهم کردن یک شی کوچک متشکل از اوراق که طی یک دقیقه از هم می‌پاشد ({{DC}} 20)
- **{{Craft}}:** {{Musket}}, {{Piston}}, {{Bell}}, {{BullseyeLantern}}, {{Flask}}, {{HoodedLantern}}, {{HuntersTrap}}, {{Lock}}, {{Manacles}}, {{Mirror}}, {{Shovel}}, {{SignalWhistle}}, {{Tinderbox}}

#### {{WeaversTools}} ({{GP:۱}}) 

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** {{LB:5}}
- **{{Utilize}}:** دوختن یک پارگی در لباس ({{DC}} 10), یا دوختن یک طرح کوچک ({{DC}} 10)
- **{{Craft}}:** {{PaddedArmor}}, {{Basket}}, {{Bedroll}}, {{Blanket}}, {{FineClothes}}, {{Net}}, {{Robe}}, {{Rope}}, {{Sack}}, {{String}}, {{Tent}}, {{TravelersClothes}}

#### {{WoodcarversTools}} ({{GP:۱}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** {{LB:5}}
- **{{Utilize}}:** حک کردن یک طرح در چوب ({{DC}} 10)
- **{{Craft}}:** {{Club}}, {{Greatclub}}, {{Quarterstaff}}, {{RangedWeapons}} (به جز {{Pistol}}, {{Musket}}, و {{Sling}}), {{ArcaneFocus}}, {{Arrows}}, {{Bolts}}, {{DruidicFocus}}, {{InkPen}}, {{Needles}}

### {{Tool}}‌های دیگر

These tools support adventure and other pursuits.
این {{Tool}}‌ها به {{Adventurer}}‌ها در کارهای دیگر کمک می‌کنند.
#### {{DisguiseKit}} ({{GP:۲۵}})

- **{{Ability}}:** {{Charisma}}
- **{{Weight}}:** {{LB:3}}
- **{{Utilize}}:** آرایش کردن ({{DC}} 10)
- **{{Craft}}:** {{Costume}}

#### {{ForgeryKit}} ({{GP:۱۵}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** {{LB:5}}
- **{{Utilize}}:** تقلید ۱۰ کلمه یا کمتر از دست خط یک نفر ({{DC}} 15), یا تکثیر یک مهر مومی ({{DC}} 20)

#### {{GamingSet}} (متفاوت)

- **{{Ability}}:** {{Wisdom}}
- **{{Weight}}:** —
- **{{Utilize}}:** تشخیص دادن تقلب کردن یک نفر ({{DC}} 10), یا بردن بازی ({{DC}} 20)
- Variants: {{Dice}} ({{SP:۱}}), {{Dragonchess}} ({{GP:۱}}), {{PlayingCards}} ({{SP:۵}}), {{ThreeDragonAnte}} ({{GP:۱}})

#### {{HerbalismKit}} ({{GP:۵}})

- **{{Ability}}:** {{Intelligence}}
- **{{Weight}}:** {{LB:3}}
- **{{Utilize}}:** شناسایی یک گیاه ({{DC}} 10)
- **{{Craft}}:** {{Antitoxin}}, {{Candle}}, {{HealersKit}}, {{PotionOfHealing}}

#### {{MusicalInstrument}} (متفاوت)

- **{{Ability}}:** {{Charisma}}
- **{{Weight}}:** متفاوت
- **{{Utilize}}:** نواختن یک آهنگ دانسته ({{DC}} 10), یا بداهه نواختن یک موسیقی ({{DC}} 15)
- Variants: {{Bagpipes}} ({{GP:۳۰}}, {{LB:6}}), {{Drum}} ({{GP:۶}}, {{LB:3}}), {{Dulcimer}} ({{GP:۲۵}}, {{LB:10}}), {{Flute}} ({{GP:۲}}, {{LB:1}}), {{Horn}} ({{GP:۳}}, {{LB:2}}), {{Lute}} ({{GP:۳۵}}, {{LB:2}}), {{Lyre}} ({{GP:۳۰}}, {{LB:2}}), {{PanFlute}} ({{GP:۱۲}}, {{LB:2}}), {{Shawm}} ({{GP:۲}}, {{LB:1}}), {{Viol}} ({{GP:۳۰}}, {{LB:1}})

#### {{NavigatorsTools}} ({{GP:۲۵}})

- **{{Ability}}:** {{Wisdom}}
- **{{Weight}}:** {{LB:2}}
- **{{Utilize}}:** کشیدن یک مسیر ({{DC}} 10), یا مشخص کردن موقعیت با ستاره‌شناسی ({{DC}} 15)

#### {{PoisonersKit}} ({{GP:۵۰}})

- **{{Ability}}:** {{Intelligence}}
- **{{Weight}}:** {{LB:2}}
- **{{Utilize}}:** تشخیص یک شی مسموم شده ({{DC}} 10)
- **{{Craft}}:** {{BasicPoison}}

#### {{ThievesTools}} ({{GP:۲۵}})

- **{{Ability}}:** {{Dexterity}}
- **{{Weight}}:** {{LB:1}}
- **{{Utilize}}:** باز کردن یک قفل ({{DC}} 15), یا خنثی کردن یک تله ({{DC}} 15)
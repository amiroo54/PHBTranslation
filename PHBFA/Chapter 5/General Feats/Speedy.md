*{{Feat}} {{General}} (پیش‌نیاز: {{Level}} ۴+، {{Dexterity}} یا {{Constitution}} ۱۳+)*

تو {{Benefits}} زیر را می‌گیری:

***افزایش {{AbilityScore}}.*** {{Score}} {{Dexterity}} یا {{Constitution}}ت را ۱ واحد، تا حداکثر ۲۰، افزایش بده. 

***افزایش {{Speed}}.*** {{Speed}}ت ۱۰ فوت زیاد می‌شود.

***{{Dash}} به روی {{DifficultTerrain}}.*** وقتی در {{Turn}}ت از {{Action}} {{Dash}} استفاده می‌کنی، برای باقی {{Turn}} برای حرکت در {{DifficultTerrain}} {{Movement}} اضافه‌ای لازم نداری.

***حرکت چست.*** {{OpportunityAttack}} نسبت به تو {{Disadvantage}} دارند.